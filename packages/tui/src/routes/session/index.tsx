...
function ToolPart(props: { last: boolean; part: ToolPart; message: AssistantMessage }) {
  ...
  // EXCEPT the Neko conversation tools, whose whole point is to be read.
  const shouldHide = createMemo(() => {
    // Board updates are pure bookkeeping mirrored on the kanban — never clutter
    // the chat with them.
    if (NEKO_HIDDEN.has(props.part.tool)) return true
    if (NEKO_CONVERSATION.has(props.part.tool)) return false
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        <Match when={NEKO_CONVERSATION.has(props.part.tool)}>
          <NekoConversation {...toolprops} />
        </Match>
        <Match when={true}>
...
// Neko multi-agent tools whose output is a conversation/transcript the user
// should always see (not collapsed like generic tool output).
const NEKO_CONVERSATION = new Set(["discuss", "review", "coimplement"])
// Neko bookkeeping tools that are reflected elsewhere (the kanban) and only add
// noise to the chat — hidden entirely.
const NEKO_HIDDEN = new Set(["board"])
const NEKO_TITLES: Record<string, string> = {
  discuss: "Discussion",
  review: "Peer review",
  coimplement: "Collaborative implementation",
}
const NEKO_PENDING: Record<string, string> = {
  discuss: "Specialists are discussing…",
  review: "Peer review in progress…",
  coimplement: "Implementing collaboratively…",
}

function NekoConversation(props: ToolProps) {
  const { theme } = useTheme()
  const output = createMemo(() => props.output?.trim() ?? "")
  const label = createMemo(() => NEKO_TITLES[props.tool] ?? props.tool)
  return (
    <Show
      when={output()}
      fallback={
        <InlineTool
          icon="◆"
          pending={NEKO_PENDING[props.tool] ?? "Working…"}
          complete={props.part.state.status === "completed"}
          spinner={props.part.state.status !== "completed"}
          part={props.part}
        >
