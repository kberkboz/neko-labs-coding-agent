...
export type AutocompleteOption = {
  ...
  onSelect?: () => void
  path?: string
  /** Neko feature — rendered in the brand accent color to stand out. */
  accent?: boolean
  /** When selected, run onSelect without closing the menu (e.g. the "more" toggle). */
  keepOpen?: boolean
}

// Neko slash commands highlighted in the autocomplete so the Neko features pop.
const NEKO_COMMANDS = new Set(["discuss", "experiment", "knowledge"])
// Neko TUI slash commands — also treated as Neko in the menu.
const NEKO_SLASH = /^\/(board|task|agents|bypass|manual-review)\b/
...
export function Autocomplete(props: {
  ...
  // When the slash menu first opens it shows only Neko commands; the "more"
  // entry flips this to reveal the opencode commands.
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    // Mark Neko TUI slashes (board/task) as accent so they group + highlight with
    // the other Neko commands.
    const results: AutocompleteOption[] = slashes().map((s) =>
      NEKO_SLASH.test(s.display.trim()) ? { ...s, accent: true } : s,
    )
    ...
    for (const serverCommand of sync.data.command) {
      ...
      results.push({
        ...
        accent: NEKO_COMMANDS.has(serverCommand.name),
        onSelect: () => {
  ...
  const options = createMemo((prev: AutocompleteOption[] | undefined) => {
    ...
    if (!searchValue) {
      // Slash menu opens showing only Neko commands; the rest (opencode) are
      // tucked behind a "more" entry. Typing still fuzzy-matches everything.
      if (store.visible === "/" && !showAllCommands()) {
        const neko = commandsValue.filter((c) => c.accent)
        const more: AutocompleteOption = {
          display: "▸ Built-in commands…",
          description: "  show all the standard commands",
          keepOpen: true,
          onSelect: () => {
            setShowAllCommands(true)
            setStore("selected", neko.length)
          },
        }
        return neko.length > 0 ? [...neko, more] : [...nonFileOptions]
      }
  ...
  function select() {
    ...
    if (!selected) return
    // The "more" toggle expands the list in place without closing the menu.
