...
import { ExperimentTool } from "./experiment"
import { TasksTool } from "./tasks"
import { ChangesetTool } from "./changeset"
import { BoardTool } from "./board"
import { CoimplementTool } from "./coimplement"
import { AcademicSearchTool } from "./academic_search"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const discusstool = yield* DiscussTool
    const reviewtool = yield* ReviewTool
    const knowledgetool = yield* KnowledgeTool
    const experimenttool = yield* ExperimentTool
    const taskstool = yield* TasksTool
    const changesettool = yield* ChangesetTool
    const boardtool = yield* BoardTool
    const coimplementtool = yield* CoimplementTool
    const academicsearchtool = yield* AcademicSearchTool
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const tool = yield* Effect.all({
          ...
          plan: Tool.init(plan),
          discuss: Tool.init(discusstool),
          review: Tool.init(reviewtool),
          knowledge: Tool.init(knowledgetool),
          experiment: Tool.init(experimenttool),
          tasks: Tool.init(taskstool),
          changeset: Tool.init(changesettool),
          board: Tool.init(boardtool),
          coimplement: Tool.init(coimplementtool),
          academic_search: Tool.init(academicsearchtool),
        })
